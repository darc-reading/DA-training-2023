...
def var3dL96(x0, t, tobs, y, H, B, R, F, gradtest=False):
    ...
    assert float(o2t) == anal_dt / deltat, \
    ...
    # np.random.seed(10)
    av_var = 0.0
    av_var = np.mean(np.diag(B))
    xb1 = x0 + np.sqrt(av_var) * np.random.randn(nx)
...
def one3dvar(xb, y, H, invB, invR, purpose="DA", state=0):
    ...
    if purpose == "DA":
        ...
        result = fsolve(gradJ, xb, xtol=1e-4)
    elif purpose == "grad":
...
def var4dL96(
    x0, t, tobs, period_obs, anawin, y_traj, H, B, R, F, TL_type, gradtest=False
):
    """4D-Var cycling data assimilation routine for Lorenz 1996 using 3D-Var
...
def one4dvar(t, xb_traj, tobs, y_traj, H, invB, invR, TL_type, purpose="DA", state=0):
    ...
    if purpose == "DA":
        ...
        # gradJ is the name of the function used to find the gradient of the cost fn
        # result is the analysis
        result = xb_traj[:, 0] + fsolve(gradJ, np.zeros([nx]), xtol=1e-4)
    elif purpose == "grad":
        # Return the gradient of the cost function
        result = gradJ(state)
    elif purpose == "cost":
